...
> 代码知识图谱索引与各目录 `AGENTS.md` 自动维护。跨模块共性见 [总览](../overview.md)。
...
以 **bundle + cli 套装** 交付；对外暴露 **`work graph`** 简易命令（对标 `codegraph init -i`）。封装上游 [CodeGraph](https://github.com/colbymchenry/codegraph) CLI（npm 全局安装），不自研多语言 AST 解析器。

## 2. 已确认决策

| 维度 | 决策 |
|------|------|
| 图谱实现 | 封装上游 `codegraph` CLI |
| 集成方式 | `examples/codegraph/installer.yaml` + `examples/codegraph-kit/bundle.yaml`（或一键 `codegraph-stack`） |
| AGENTS.md 范围 | 仅 CodeGraph 已索引的**代码目录**（`nodeCount > 0` 且语言为 go/ts/js/py 等，排除 yaml/md/docs/examples） |
| 复杂度过滤 | 单文件且符号少于 12 个不生成（`AGENTS_MIN_SYMBOLS`）；`cmd/*` 入口始终保留 |
...
work install codegraph-kit    → 安装 Skill + MCP（--scope project 时 post_install → graph init）
...
`internal/graph/runner.go` 跑 `init`/`sync`/`status` 并接好自动同步 hook。
...
- **关键符号** — 该目录下导出函数/类型（来自图谱，带 `file:line`）
- **相关目录** — 父目录、子包、常见协作目录

页眉标注自动生成与更新命令（手动 `generate-agents.sh`、自动 `setup-auto-sync.sh`）。
